...
class Response(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["ad", "author"],
                name="unique_response_per_user_per_ad",
            )
        ]
        indexes = [
    ...
    def __str__(self):
        return f"Отклик {self.author} на {self.ad}"

    def clean(self):
        if self.ad.author_id == self.author_id:
    ...
    def accept(self):
        ...
        send_mail(
            subject=f"Ваш отклик принят: {self.ad.title}",
            message=(
                f"Ваш отклик на объявление '{self.ad.title}' был принят."
            ),
